...
public static class Scripts
{
    ...
    public static void CreateGameManager(SceneJson scene)
    {
        ...
        using (StreamWriter outfile = new StreamWriter(path))
        {
            ...
            outfile.WriteLine("        {");
            outfile.WriteLine("            Mouse = Input.mousePosition;");
            outfile.WriteLine("            Ray ray = mainCamera.ScreenPointToRay(Input.mousePosition);");
            outfile.WriteLine("            if (new Plane(Vector3.up, Vector3.zero).Raycast(ray, out float enter))");
