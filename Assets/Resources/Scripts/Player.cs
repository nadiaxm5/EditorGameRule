...
public class Player : MonoBehaviour {
    ...
    public float desp=7f;
    public float despX=0.28f;
    public float despY=0.3f;
    public float despZ=0.7f;
    ...
    void FixedUpdate(){
        ...
        if(Condition.Compare("this.health<0",scopeList)){
            ...
            Action.Edit("this.moving","2",scopeList);
        ...
        if(Condition.Compare("this.moving==2",scopeList)){
    ...
    void Update(){
        ...
        if(Condition.Touch("press","false",gameObject)){
            Action.Spawn("Bullet", gameObject, "this.despX", "this.despY", "this.despZ", "0", "0", "0", scopeList);
            Action.Spawn("Laser", gameObject, "this.despX", "this.despY", "this.despZ", "0", "0", "0", scopeList);
    ...
    void Start() {
        scopeList = Utils.CreateScope(gameObject.GetInstanceID(),"RotateTo(this.rotSpeed,#MouseWorld.x,#MouseWorld.y,#MouseWorld.z,this.x,this.y,this.z);Edit(#CameraPosition.x,this.x);Edit(#CameraPosition.z,this.z-this.desp);Collision(Hellephant);Edit(this.health,this.health-Hellephant.damage);PlaySound(PlayerHurt);Collision(ZomBear);Edit(this.health,this.health-ZomBear.damage);Collision(ZomBunny);Edit(this.health,this.health-ZomBunny.damage);Compare(this.health<0);Edit(GameOver.Active,1);PlaySound(PlayerDeath);Edit(this.moving,2);Edit(this.speed,0);Compare(this.health==this.lastHealth);Edit(DamageCanvas.Active,0);Compare(this.health<this.lastHealth);Edit(DamageCanvas.Active,1);Edit(this.lastHealth,this.health);Compare(this.moving==0);Animate(Idle);Compare(this.moving==1);Animate(Move);Compare(this.moving==2);Animate(Death);Keyboard(RightArrow,press);Move(this.speed, 0, 90, 0);Edit(this.moving,1);Keyboard(LeftArrow,press);Move(this.speed, 0, -90, 0);Keyboard(UpArrow,press);Move(this.speed, 0, 0, 0);Keyboard(DownArrow,press);Move(this.speed, 0, 180, 0);Keyboard(RightArrow,up);Keyboard(LeftArrow,up);Keyboard(UpArrow,up);Keyboard(DownArrow,up);Edit(this.moving,0);Touch(press,false);Spawn(Bullet,this,this.despX,this.despY,this.despZ);Spawn(Laser,this,this.despX,this.despY,this.despZ);Edit(ShotLight.Active,1);PlaySound(PlayerGunShot);PlayParticles(GunBarrelEnd)");
    ...
    void Awake() {
        propertyList = Utils.CreateProperties("speed=5;rotSpeed=360;lastHealth=100.0;health=100.0;score=0;desp=7;despX=0.28;despY=0.3;despZ=0.7;moving=0");
