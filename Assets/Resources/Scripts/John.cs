...
public class John : MonoBehaviour, IGameRuleActor {
    ...
    void Start() {
        scopeList = Utils.CreateScope(gameObject.GetInstanceID(),"Edit(#CameraPosition.x,this.x);Edit(#CameraPosition.z,this.z-this.offset);Keyboard(RightArrow,press);Move(this.speed,0,90,0);Edit(this.ry,90);Edit(this.moving,1);Keyboard(LeftArrow,press);Move(this.speed,0,-90,0);Edit(this.ry,-90);Keyboard(UpArrow,press);Move(this.speed,0,0,0);Edit(this.ry,0);Keyboard(DownArrow,press);Move(this.speed,0,180,0);Edit(this.ry,180);Keyboard(RightArrow,up);Keyboard(LeftArrow,up);Keyboard(UpArrow,up);Keyboard(DownArrow,up);Edit(this.moving,0);Collision(Enemy);Edit(Caught.Active,1);Collision(End);Edit(Won.Active,1);Check(this.moving);Animate(John_Idle);Animate(John_Walk);PlaySound(Footsteps)");
