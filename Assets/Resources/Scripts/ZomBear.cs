...
public class ZomBear : MonoBehaviour {
    ...
    void FixedUpdate(){
        ...
        if(Condition.Collision("Bullet",gameObject)){
            ...
            Action.PlaySound("ZomBearHurt",gameObject);
            Action.PlayParticles("HitParticles",gameObject);
        }
        if(Condition.Compare("this.health<=0",scopeList)){
            Action.Edit("Player.score","Player.score+10",scopeList);
            Action.Spawn("ZomBearDead", gameObject, "0", "0", "0", "0", "0", "0", scopeList);
    ...
    void Start() {
        scopeList = Utils.CreateScope(gameObject.GetInstanceID(),"Edit(this.moving,1);NavigateTo(this.speed,Player.x,Player.y,Player.z);Compare(Player.health<0);Edit(this.moving,0);Edit(this.speed,0);Collision(Bullet);Edit(this.health,this.health-Bullet.damage);PlaySound(ZomBearHurt);PlayParticles(HitParticles);Compare(this.health<=0);Edit(Player.score,Player.score+10);Spawn(ZomBearDead,this);Delete(this);Compare(this.moving==0);Animate(Idle);Compare(this.moving==1);Animate(Move)");
    ...
    void Awake() {
        propertyList = Utils.CreateProperties("speed=3;health=100;damage=0.15;moving=0");
