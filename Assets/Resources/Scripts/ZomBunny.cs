...
public class ZomBunny : MonoBehaviour {
    ...
    void Start() {
        scopeList = Utils.CreateScope(gameObject.GetInstanceID(),"Edit(this.moving,1);NavigateTo(this.speed,Player.x,Player.y,Player.z);Compare(Player.health<0);Edit(this.moving,0);Edit(this.speed,0);Collision(Bullet);Edit(this.health,this.health-Bullet.damage);PlaySound(ZomBunnyHurt);PlayParticles(HitParticles);Compare(this.health<=0);Edit(Player.score,Player.score+10);Spawn(ZomBunnyDead,this);Delete(this);Compare(this.moving==0);Animate(Idle);Compare(this.moving==1);Animate(Move)");
