...
public class HellephantSpawner : MonoBehaviour {
    ...
    void Start() {
        scopeList = Utils.CreateScope(gameObject.GetInstanceID(),"Edit(this.currentTime,this.currentTime+1);Compare(this.currentTime==this.spawnTime);Edit(this.currentTime,0);Spawn(Hellephant,this)");
        if (Active) gameObject.SetActive(true);
        else gameObject.SetActive(false);
    }
    void Awake() {
        propertyList = Utils.CreateProperties("spawnTime=300.0;currentTime=0.0");
    }
}
